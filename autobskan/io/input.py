...
def str_to_range(X, remove_overlap = True, include_endpoint = True) -> list:
    ...
    if "," not in X:
        ...
        if len(X_float) == 2:
            ...
            assert len(X_float) == 3, f"If it is divided by '_' symbol, start and end value must be given"
...
class Options:
    ...
    def __init__(self, filename = None):
        ...
        ## ISO_AUTO and ISO
        current_iso_auto = self.option_dict["ISO_AUTO"].upper()
        if current_iso_auto.startswith("L"):
            self.option_dict["ISO_AUTO"] = "LOGSCALE"
            self.option_dict["ISO"] = self.keywords_info["ISO"][1] # It will be automatically chosen...
        else:
            self.option_dict["ISO_AUTO"] = TorF(current_iso_auto)
            if self.option_dict["ISO_AUTO"]:
                assert len(self.option_dict["ISO"]) == 1, "When ISO_AUTO==True, you need to specify integer in ISO"
                self.option_dict["ISO"] = int(self.option_dict["ISO"][0])
            else:
